...
class MaixVisionSystem:
    ...
    def initialize_modules(self):
        try:
            ...
            try:
                # 优先使用官方API识别器
                from src.vision.recognition.official_face_recognizer import OfficialFaceRecognizer
                self.recognizer = OfficialFaceRecognizer(max_persons=self.max_persons)
                print("✅ 官方FaceRecognize识别器初始化完成")
            except Exception as e:
                print(f"⚠️ 官方识别器初始化失败: {e}")
                print("🔄 尝试使用简化识别器...")
                try:
                    from src.vision.recognition.simple_face_recognition import SimpleFaceRecognizer
                    self.recognizer = SimpleFaceRecognizer(max_persons=self.max_persons)
                    print("✅ 简化人脸识别器初始化完成")
        ...
        while self.running and (_maix_app is None or not _maix_app.need_exit()):
            ...
            if self.disp is not None:
                try:
                    ...
                    # 处理连续采集（如果正在进行）
                    if self.recognizer and hasattr(self.recognizer, 'is_collecting') and self.recognizer.is_collecting:
                        try:
                            is_still_collecting, current_count, target_count = self.recognizer.process_continuous_collection(img)
                            
                            # 如果采集完成，更新缩略图显示
                            if not is_still_collecting and current_count == target_count:
                                # 采集完成，设置缩略图为新注册的人物
                                if hasattr(self.recognizer, 'collecting_person_id') and self.recognizer.collecting_person_id:
                                    self.current_thumbnail_person = self.recognizer.collecting_person_id
                        except Exception as e:
                            print(f"⚠️ 连续采集处理失败: {e}")
    ...
    def _handle_record_add(self):
        ...
        # 检查是否已经在采集中
        if hasattr(self.recognizer, 'is_collecting') and self.recognizer.is_collecting:
            print("⚠️ 正在采集数据中，请等待完成")
            return
        ...
        try:
            # 启动连续采集
            success, message = self.recognizer.start_continuous_collection(img)
            
            if success:
                print(f"🚀 {message}")
                print(f"📸 开始连续采集，请保持人脸在画面中...")
                print(f"📊 将采集 {self.recognizer.target_samples} 个样本以提高识别精度")
            else:
                print(f"✗ 启动采集失败: {message}")
            ...
            if detections:
                try:
                    ...
                    for detection in detections:
                        ...
                        # 检查是否正在采集数据
                        is_collecting = hasattr(self.recognizer, 'is_collecting') and self.recognizer.is_collecting
                        
                        # 根据识别结果绘制不同的标识
                        if person_name and person_name != "unknown" and confidence > 0.75:
                            # 已知人物 - 绿色边框和名称 (A, B, C)
                            img.draw_rect(x, y, w, h, color=green_color, thickness=2)
                            img.draw_string(x, max(y-20, 0), f"{person_name}", color=green_color, scale=1.2)
                            # 添加置信度显示
                            conf_text = f"({confidence:.2f})"
                            img.draw_string(x, max(y-5, 0), conf_text, color=green_color, scale=0.8)
                        elif is_collecting:
                            # 正在采集数据 - 蓝色边框和采集状态
                            blue_color = _image.Color.from_rgb(0, 100, 255)
                            img.draw_rect(x, y, w, h, color=blue_color, thickness=3)
                            img.draw_string(x, max(y-35, 0), "RECORDING", color=blue_color, scale=1.0)
                            
                            # 显示采集进度
                            if hasattr(self.recognizer, 'collection_count') and hasattr(self.recognizer, 'target_samples'):
                                progress = f"{self.recognizer.collection_count}/{self.recognizer.target_samples}"
                                img.draw_string(x, max(y-20, 0), progress, color=blue_color, scale=0.9)
                                
                                # 显示即将成为的人物名称
                                if hasattr(self.recognizer, 'collecting_person_id'):
                                    collecting_info = self.recognizer.registered_persons.get(self.recognizer.collecting_person_id, {})
                                    future_name = collecting_info.get('name', '?')
                                    img.draw_string(x, max(y-5, 0), f"→ {future_name}", color=blue_color, scale=0.8)
                        else:
                            # 未知人物 - 黄色边框和unknown标识
                            img.draw_rect(x, y, w, h, color=yellow_color, thickness=2)
                            img.draw_string(x, max(y-20, 0), "unknown", color=yellow_color, scale=1.0)
                            # 提示可以点击ADD按钮添加
                    ...
                    for detection in detections:
                        ...
                        if face_bbox:
                            ...
                            try:
                                ...
                                # 选择文字颜色和内容
                                if person_id is not None and person_name != "unknown" and confidence > 0.60:
                                    # 识别到已保存人物 - 显示人物名称
                                    ...
                                    img.draw_string(face_x, max(face_y - 45, 0), text, 
                                                  color=color, scale=1.3)
                                    # 绘制置信度（较小字体）
                                    img.draw_string(face_x, max(face_y - 25, 0), confidence_text, 
                                                  color=color, scale=0.8)
                                else:
                                    # 未识别人物 - 显示"unknown"和调试信息
                                    color = _image.Color.from_rgb(255, 165, 0)  # 橙色
                                    text = "unknown"
                                    confidence_text = f"({confidence:.3f})"
